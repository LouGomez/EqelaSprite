
/*
 * Main.eq
 * Generated by Eqela Studio 2.0b7.4
 */

public class Main : SEScene
{
	SESprite fg;
	SESprite bg;

	bool move_right=true;
	bool move_left=false;
	bool move_up=false;
	bool move_down=true;
	int speed=50;
	double rotation=0;
	SESprite text;
	int bounce=0;
	String my_text;
	SESprite my_image;

	public void initialize(SEResourceCache rsc) {
		base.initialize(rsc);
		my_text = "BOUNCE = %d".printf().add(Primitive.for_integer(bounce)).to_string();
		bg = add_sprite_for_color(Color.instance("#66B2FF"), get_scene_width(), get_scene_height());
		bg.move(0,0);
		//fg = add_sprite_for_color(Color.instance("black"), get_scene_width()*0.5, get_scene_height()*0.5);
		//fg.move(100,100);
	
		rsc.prepare_font("myfont","arial bold color=black",50);
		text=add_sprite_for_text(my_text, "myfont");
		text.move(0,get_scene_height()-text.get_height());
		rsc.prepare_image("temp","akatsuki",get_scene_width()*0.1, get_scene_height()*0.1);
		fg = add_sprite_for_image(SEImage.for_resource("temp"));
		fg.move(100,100);
	}



	public void update(TimeVal now, double delta) {
		base.update(now,delta);
		rotation+=MathConstant.M_PI/10;
		if(move_right) {
	   fg.move(fg.get_x()+delta*speed,fg.get_y());
        rotation+=MathConstant.M_PI_4;
		fg.set_rotation(rotation);
		if(fg.get_x()+fg.get_width()>get_scene_width()){
		move_right=false;
		move_left=true;
		speed=100;
		bounce++;

		} 
		my_text = "BOUNCE = %d".printf().add(Primitive.for_integer(bounce)).to_string();
		text.set_text(my_text, "myfont");
		speed+=50;
		
        }
			
   if(move_left) {
	    fg.move(fg.get_x()-delta*300,fg.get_y());
        rotation+=MathConstant.M_PI_4;
      fg.set_rotation(rotation);
	    if(fg.get_x()<0){
	    move_right=true;
	    move_left=false;
    	speed=50;
        bounce++;

		}
		my_text = "BOUNCE = %d".printf().add(Primitive.for_integer(bounce)).to_string(); 
		text.set_text(my_text, "myfont");
		speed+=50;
    	}

  if(move_down) {
		fg.move(fg.get_x(), fg.get_y()+delta*500);
		rotation+=MathConstant.M_PI_4;
		fg.set_rotation(rotation);
		if(fg.get_y()+fg.get_height()>get_scene_height()) {
		move_down=false;
		move_up=true;
		speed=50;
		bounce++;
 
		} 
		my_text = "BOUNCE = %d".printf().add(Primitive.for_integer(bounce)).to_string();
		text.set_text(my_text, "myfont");
	    speed+=50;
		}

   if(move_up) {
    	fg.move(fg.get_x(), fg.get_y()-delta*300);
    	if(fg.get_y()<0) {
	    move_down=true;
	    move_up=false;
    	speed=50;
        bounce++;

		} 
		my_text = "BOUNCE = %d".printf().add(Primitive.for_integer(bounce)).to_string();
		text.set_text(my_text, "myfont");
		speed+=50;
        }
}

	public void set_rotation(double angle) {
		rotation=angle;
	}

	public double get_rotation() {
		return(rotation);
	}


public void cleanup() {
	base.cleanup();
	SESprite.remove(fg);
	SESprite.remove(bg);
	SESprite.remove(text);
	}

	}
	